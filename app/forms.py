...
from wtforms import StringField, PasswordField, DateField, SubmitField, SelectField, IntegerField, BooleanField, Field
...
def validate_email(form, email):
    ...
    if user is not None:
        raise ValidationError(_("Such email is already in use."))
...
class RegistrationForm(FlaskForm):
    ...
    username = StringField('Username', validators=[DataRequired()])
    password = PasswordField('Password', validators=[DataRequired()])
    password_repeat = PasswordField('Password Repeat', validators=[DataRequired(), EqualTo('password', message=_('Passwords must match'))])
...
class SearchForm(FlaskForm):
    ...
    def validate_adults(adults):
        if adults < 0 or adults > 10:
            raise ValidationError(_("Please pick the number of adults from 0 to 10"))

    def validate_teens(teens):
        if teens < 0 or teens > 10:
            raise ValidationError(_("Please pick the number of teens from 0 to 10"))

    def validate_children(children):
        if children < 0 or children > 10:
            raise ValidationError(_("Please pick the number of children from 0 to 10"))

    def validate_infants(infants):
        if infants < 0 or infants > 10:
            raise ValidationError(_("Please pick the number of infants from 0 to 10"))

    def validate_seniors(seniors):
        if seniors < 0 or seniors > 10:
            raise ValidationError(_("Please pick the number of seniors from 0 to 10"))
